    ...
    while (1)
    {
        ...
        if (ch != '\n')
        {
            ...
            else if (count >= 32)
            {
                for (int i = 0; i < 15; i++)
                {
                    line1_buffer[i] = line1_buffer[i + 1];
                }

                line1_buffer[15] = line2_buffer[0];
                line1_buffer[16] = '\0';

                for (int i = 0; i < 15; i++)
                {
                    line2_buffer[i] = line2_buffer[i + 1];
                }
                line2_buffer[15] = ch;
                line2_buffer[16] = '\0';
            }
        ...
        else
        {
            if (count <= 17)
            {
                clcd_print("                ", (0x80 + 0));
                clcd_print("                ", (0xC0 + 0));
                clcd_print(line1_buffer, (0x80 + 0));
            }
            else
            {
                clcd_print("                ", (0x80 + 0));
                clcd_print("                ", (0xC0 + 0));
                clcd_print(line1_buffer, (0x80 + 0));
                clcd_print(line2_buffer, (0xC0 + 0));
            }
